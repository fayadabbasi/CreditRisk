...
class RandFor:
    ...
    def randfor_action(self, xtr, xte, ytr, yte, tr=0.5, class_weight={0:10, 1:1}, n_estimators=5, max_depth=2):    
        ...
        OUTPUT: df_actual_predicted_probs, fpr, tpr, thresholds, auroc, score, features
        '''
        y_glb = ytr
        ...
        features = pd.DataFrame(rfit.feature_importances_,index = xtr.columns, columns=['importance']).sort_values('importance',ascending=False)
        ...
        return df_actual_predicted_probs, fpr, tpr, thresholds, auroc, score, features, rfit
...
if __name__ == '__main__':
    ...
    df_actual_predicted_probs, fpr, tpr, thresholds, auroc, score, features, rfit = r.randfor_action(X_train_woe, X_test_woe, y_train.iloc[:,1], y_test.iloc[:,1], tr=0.17, class_weight={0:1, 1:20}, n_estimators=70, max_depth=None)
